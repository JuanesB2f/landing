...
const _id_ = defineEventHandler(async (event) => {
  ...
  if (method === "PUT") {
    try {
      ...
      const payload = {};
      if (body.user_id !== void 0) payload.user_id = body.user_id;
      ...
      const { data, error } = await supabase.from("user_offers").update(payload).eq("id_offer", id).select().single();
  ...
  if (method === "DELETE") {
    try {
      await requireAdmin(event);
      const { error } = await supabase.from("user_offers").delete().eq("id_offer", id);
