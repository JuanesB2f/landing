import { d as defineEventHandler, s as serverSupabaseClient, g as getMethod, h as getRouterParam, n as createError, a as requireAdmin, b as respondSuccess, i as getHeader, j as readMultipartFormData, k as readBody, r as respondError } from '../../../_/nitro.mjs';
...
const _id_ = defineEventHandler(async (event) => {
  ...
  try {
    switch (method) {
      case "GET":
        const { data: product, error: getError } = await client.from("products").select(`
            *,
            category:categories(name)
          `).eq("id_product", id).single();
        if (getError) {
      ...
      case "PUT":
        ...
        const { data: updatedProduct, error: updateError } = await client.from("products").update({
          ...
          is_active: updateBody.is_active !== void 0 ? updateBody.is_active : true,
      ...
      case "DELETE":
        await requireAdmin(event);
        const { error: deleteError } = await client.from("products").delete().eq("id_product", id);
        if (deleteError) {
