...
const createFromUser = defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (profile.role !== "user") return respondError("Solo usuarios con rol user pueden crear aqu\xED");
    ...
    const { data: offers } = await supabase.from("offers").select("product_id, discount_percent, is_active, valid_from, valid_to").eq("is_active", true);
...
export { createFromUser as default };
